export default class FormValidator {
    ...
    clearSpanError(form) {
        ...
        const inputPopupForClear = form.querySelectorAll(this._config.inputSelector);
        inputPopupForClear.forEach((PopupForClear) => PopupForClear.classList.remove(this._config.errorClass));
        error.forEach((error) => {
            if (error) {
                error.textContent = "";
            }
